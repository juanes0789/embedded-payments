...
public class RegisterUserUseCase {
    ...
    public RegisteredUser execute(String email, String password, String role, String merchantName, String contactName, String contactEmail) {
        ...
        boolean requiresMerchantProfile = !USER.equals(normalizedRole);
        if (requiresMerchantProfile && (merchantName == null || merchantName.isBlank())) {
            throw new DomainException(
                    HttpStatus.BAD_REQUEST,
                    "MERCHANT_NAME_REQUIRED",
                    "merchantName is required",
                    List.of("role: " + normalizedRole)
            );
        }
        ...
        if (!USER.equals(normalizedRole)) {
            boolean activateOnRegistration = MERCHANT.equals(normalizedRole);
            RegisterMerchantUseCase.RegisteredMerchant registeredMerchant =
                    registerMerchantUseCase.execute(
                            merchantName,
                            normalizedEmail,
                            contactName,
                            contactEmail,
            ...
            merchantStatus = registeredMerchant.merchant().getStatus();
        }
    ...
    private String normalizeRole(String role) {
        ...
        if (!ADMIN.equals(normalized) && !MERCHANT.equals(normalized) && !USER.equals(normalized)) {
            throw new DomainException(
                    HttpStatus.BAD_REQUEST,
                    "INVALID_ROLE",
                    "Role must be ADMIN, MERCHANT or USER",
                    List.of("role: " + role)
            );
        }
        return normalized;
